...
from equations.artificial_intelligence.shared.capability_artifacts import CAPABILITIES, run_capability
    ...
    *,
    ...
    for offset, capability in enumerate(CAPABILITIES):
        capability_results[capability] = run_capability(
            ...
            producer_module=f"equations.artificial_intelligence.{architecture}.{mode}.run_all_capabilities",
        )
    ...
    manifest = write_metadata(
        output / f"{architecture}_{mode}_six_output_manifest.json",
        {
            ...
            "regeneration_command": (
                f"python -m equations.artificial_intelligence.{architecture}.{mode}.run_all_capabilities"
            ),
