...
def run_suite(category: str, output_dir: str | Path, *, seed: int = 0) -> dict[str, Path | list[Path]]:
    ...
    for contract, evaluation in evaluations:
        ...
        module_name = {
            "dfi": "dynamic_fluctuation_index",
            "pdfi": "parity_dfi",
            "elastic_pi": "elastic_pi",
            "elastic_pi_norm": "elastic_pi_norm",
        }[category]
        manifests.append(
            write_artifact_manifest(
                ...
                build_manifest(
                    ...
                    regeneration_command=f"python -m equations.{module_name}.simulation.run_contract_suite --output {output.as_posix()}",
                ),
            )
        )
