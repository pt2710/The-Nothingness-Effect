...
def signed_polar_field(value: SignedPolarFieldInput) -> SignedPolarFieldResult:
    if value.horizon <= 0 or not value.times:
        raise DomainViolationError("positive horizon and nonempty spatial times are required")
    ...
    theta = tuple(2 * pi_result.value * time / value.horizon for time in value.times)
    field = tuple(radius * phase * np.exp(1j * item) for item in theta)
    gradients = tuple(field[index + 1] - field[index] for index in range(len(field) - 1))
    expected_initial = radius * phase * np.exp(1j * theta[0])
    boundary = float(abs(field[0] - expected_initial))
    reconstruction = float(
        np.linalg.norm(np.asarray(field) - field[0] * np.exp(1j * (np.asarray(theta) - theta[0])))
    )
    closed = boundary <= 1e-10 and reconstruction <= 1e-10
...
def field_residual(_source, output: SignedPolarFieldResult) -> ResidualResult:
    vector = (output.boundary_trace_residual, output.reconstruction_residual)
    ...
    return ResidualResult(
        ...
        1e-10,
        norm <= 1e-10,
        ClosureStatus.NUMERICAL_CANDIDATE if norm <= 1e-10 else ClosureStatus.OPEN,
        metadata={"candidate_not_minimizer": True},
    )
...
def remove_arithmetic_source(value: SignedPolarFieldInput) -> SourceRemovalResult:
    ...
    removed = signed_polar_field(
        SignedPolarFieldInput(1.0, value.phase_unit, value.order, value.damping, value.times, value.horizon)
    )
...
def remove_geometry_source(value: SignedPolarFieldInput) -> SourceRemovalResult:
    complete = signed_polar_field(value)
    removed = np.full(len(value.times), complete.canonical_radius * complete.canonical_phase_unit, dtype=complex)
    return source_removal_result(
...
def contracts() -> tuple[ComplexContract, ...]:
    return (
        ComplexContract(
            ComplexId("addition_of_the_two_complete_b_level_operator_families"),
            APPENDIX,
            APPENDIX_SHA256,
            ComplexLevel.C,
            (
            ...
            DomainSpec("signed-polar spatial field", "quotient-compatible radius, phase, and spatial times", (SignedPolarFieldInput,)),
            CodomainSpec("calibrated complex field", "field, local gradients, boundary and reconstruction residuals", (SignedPolarFieldResult,)),
            signed_polar_field,
            ...
            artifact_spec=ArtifactSpec(("field_csv", "boundary_plot", "animation_generator", "operator_diagram"), "python -m equations.mathematical_closure.simulation"),
            exact_semantics=False,
            implementation_path="equations/mathematical_closure/c_level.py",
        ),
    )
