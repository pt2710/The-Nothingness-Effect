...
def _arguments(
    ...
    *,
    ...
    final_qa = _write(tmp_path / "final.json", {"final_qa_passed": True, "release_blockers": [], "theorem_inventory": {"total": 1}, "repository_result_commit": evidence_commit})
    archive_qa = _write(tmp_path / "archive.json", {"passed": archive_passed, "actual_archive_sha256": "a" * 64 if archive_passed else None, "expected_archive_sha256": "a" * 64})
    status = _write(tmp_path / "status.json", {
        "rows": 1,
        "dimensions": {
            "runtime_status": {"implemented": 1}, "source_exactness": {"exact": 1},
            "closure_status": {"open": 1}, "validation_status": {"synthetic": 1},
            "artifact_status": {"complete_core_bundle": 1},
        },
        "open_and_numerical_candidate_preserved": 1, "artifact_core_bundle_gaps": 0,
    })
    matrix = _write(tmp_path / "matrix.json", {"summary": {"rows": 1}})
    provenance = _write(tmp_path / "provenance.json", {"repository_result_commit": evidence_commit, "manifests": [{"theorem_complex_id": "fixture"}]})
    artifact_coverage = _write(tmp_path / "artifact_coverage.json", {"theorem_complexes": 1, "complete_core_artifact_bundles": 1 if artifact_passed else 0, "diagnostic_bundles_enriched": 1, "passed": artifact_passed})
    closure_obligations = _write(tmp_path / "closure_obligations.json", {"open_or_numerical_candidate_count": 1 if ledger_complete else 0, "counts": {"open": 1 if ledger_complete else 0}, "all_open_states_represented": ledger_complete})
    robustness = _write(tmp_path / "robustness.json", {"seeds": list(robustness_seeds), "epochs": 3, "scenarios": ["clean", "remove_color", "remove_sound", "remove_vision"], "records": len(robustness_seeds) * 4, "all_metrics_finite": True, "claim_boundary": "synthetic multi-seed robustness evidence"})
    convergence = _write(tmp_path / "convergence.json", {"families": ["elastic_dubler", "locality_driven_gravity", "black_hole_dynamics", "elastic_pi_ripples"], "resolutions": [9, 17, 33], "records": convergence_records, "all_metrics_finite": True, "claim_boundary": "finite grid-refinement diagnostic; not continuum convergence proof"})
    equation_alignment = _write(tmp_path / "equation_alignment.json", {"rows": alignment_rows, "complete_mappings": alignment_rows, "source_faithful_corrections": 23, "first_labels": alignment_rows, "equation_labels": max(2, alignment_rows), "errors": [], "passed": alignment_rows == 1})
    ...
    return argparse.Namespace(
        final_qa=final_qa, archive_qa=archive_qa, status_dimensions=status,
        matrix_report=matrix, provenance=provenance, artifact_coverage=artifact_coverage,
        closure_obligations=closure_obligations, multimodal_robustness=robustness,
        source_faithful_convergence=convergence,
        equation_implementation_alignment=equation_alignment,
        archive_manifest=archive_manifest, recertification_manifest=recertification,
        result_commit=RESULT_COMMIT, branch="fixture", ci_conclusion="success", ci_run_id="1",
        output=tmp_path / "immutable.json", check=True,
    )
