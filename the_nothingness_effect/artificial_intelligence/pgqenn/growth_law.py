...
@dataclass(frozen=True)
class SignedTriadicGrowthGraph:
    """TNE Flowpoint lift of the positive MPL-TC finite stream graph.

    MPL-TC remains authoritative only for the positive placements.  The paired
    negative values are a TNE neural carrier generated by the involution
    ``n -> -n``.  Geometry and edge weights may differ across the two spectra;
    only the typed value pairing is required to be involutive.
    """

    values: tuple[int, ...]
    spectrum_signs: tuple[int, ...]
    stream_kinds: tuple[TCStreamKind, ...]
    source_prime_indices: tuple[int, ...]
    involution_partner_indices: tuple[int, ...]
    coordinates_3d: torch.Tensor
    adjacency: torch.Tensor
    spatial_adjacency: torch.Tensor
    ...
    dependency_sha256: str
    axis_labels: tuple[str, str, str] = (
        "signed_number_spectrum",
        "tc_stream_kind",
        "asymmetric_structural_depth",
    )
    ...
    def __post_init__(self) -> None:
        ...
        aligned = (
            ...
            self.stream_kinds,
            self.source_prime_indices,
            self.involution_partner_indices,
        )
        if self.positive_node_count < 4 or node_count != 2 * self.positive_node_count:
            raise ValueError("signed growth requires paired positive/negative nodes")
        if any(len(field) != node_count for field in aligned):
            raise ValueError("signed growth fields must align")
        if set(self.spectrum_signs) != {-1, 1}:
            raise ValueError("signed growth must contain both number spectra")
        if self.coordinates_3d.shape != (node_count, 3):
            raise ValueError("signed growth coordinates must have shape [nodes, 3]")
        for name, value in (
            ("adjacency", self.adjacency),
            ("spatial adjacency", self.spatial_adjacency),
        ):
            if value.shape != (node_count, node_count):
                raise ValueError(f"signed growth {name} has the wrong shape")
            if not bool(torch.isfinite(value).all()) or not torch.allclose(value, value.T):
                raise ValueError(f"signed growth {name} must be finite and symmetric")
            if not bool((torch.diagonal(value) == 0).all()):
                raise ValueError(f"signed growth {name} cannot contain self loops")
        if not bool(torch.isfinite(self.coordinates_3d).all()):
            raise ValueError("signed growth coordinates contain NaN or infinity")
        for index, partner in enumerate(self.involution_partner_indices):
            if self.involution_partner_indices[partner] != index:
                raise ValueError("signed growth partner map must square to identity")
            if self.values[index] + self.values[partner] != 0:
                raise ValueError("signed growth partner values must negate")

    @property
    def spectrum_counts(self) -> dict[str, int]:
        return {
            "positive": self.spectrum_signs.count(1),
            "negative": self.spectrum_signs.count(-1),
        }

    @property
    def value_involution_residual(self) -> float:
        return float(
            max(
                abs(value + self.values[partner])
                for value, partner in zip(
                    self.values, self.involution_partner_indices, strict=True
                )
            )
        )

    @property
    def coordinate_asymmetry(self) -> float:
        count = self.positive_node_count
        positive = self.coordinates_3d[:count]
        negative = self.coordinates_3d[count:]
        return float(torch.linalg.vector_norm(positive + negative))
...
class PrimeGraph:
    ...
    signed_triadic_growth: SignedTriadicGrowthGraph | None = None
    ...
    def __post_init__(self) -> None:
        ...
        if self.growth_mode == "mpl_tc_prime_motif" and self.signed_triadic_growth is None:
            raise ValueError(
                "canonical PGQENN growth requires the TNE signed-spectrum bridge"
            )
...
def signed_triadic_growth_graph(
    positive: TriadicGrowthGraph,
) -> SignedTriadicGrowthGraph:
    """Lift a verified positive MPL-TC prefix through Flowpoint ``n -> -n``.

    This is intentionally not a claim that MPL-TC defines negative integers.
    The negative block is an asymmetric, deterministic PGQENN specialization,
    while the value partner map remains an exact involution.
    """

    count = len(positive.values)
    positive_values = positive.values
    values = (*positive_values, *(-value for value in positive_values))
    signs = (1,) * count + (-1,) * count
    kinds = (*positive.stream_kinds, *positive.stream_kinds)
    source_indices = (
        *positive.source_prime_indices,
        *positive.source_prime_indices,
    )
    partners = tuple(range(count, 2 * count)) + tuple(range(count))

    base = positive.coordinates_3d
    magnitude = 0.12 + 0.88 * (base[:, 0] + 1.0) / 2.0
    stream = base[:, 1]
    structural = base[:, 2]
    positive_coordinates = torch.stack((magnitude, stream, structural), dim=-1)
    # The negative realization grows on the same three typed axes but is not
    # constrained to be a mirror image of the positive embedding.
    negative_coordinates = torch.stack(
        (
            -magnitude * (0.82 + 0.08 * (stream + 1.0)),
            stream + 0.12 * structural,
            -0.72 * structural + 0.18 * stream,
        ),
        dim=-1,
    )
    coordinates = torch.cat((positive_coordinates, negative_coordinates), dim=0)

    adjacency = torch.zeros((2 * count, 2 * count), dtype=base.dtype)
    adjacency[:count, :count] = positive.adjacency
    negative_factor = 0.68 + 0.14 * (stream + 1.0) / 2.0 + 0.08 * (
        structural + 1.0
    ) / 2.0
    negative_weights = torch.sqrt(
        negative_factor.unsqueeze(1) * negative_factor.unsqueeze(0)
    )
    adjacency[count:, count:] = positive.adjacency * negative_weights
    for index in range(count):
        bridge_weight = 0.52 + 0.14 * (1.0 - abs(float(stream[index])))
        adjacency[index, count + index] = bridge_weight
        adjacency[count + index, index] = bridge_weight
    spatial = spatial_locality_adjacency(adjacency, coordinates)
    return SignedTriadicGrowthGraph(
        values=values,
        spectrum_signs=signs,
        stream_kinds=kinds,
        source_prime_indices=source_indices,
        involution_partner_indices=partners,
        coordinates_3d=coordinates,
        adjacency=adjacency,
        spatial_adjacency=spatial,
        positive_node_count=count,
        dependency_sha256=positive.dependency_sha256,
    )
...
class CanonicalPrimeGrowth:
    ...
    def build(self, node_count: int, *, dtype: torch.dtype = torch.float32) -> PrimeGraph:
        ...
        signed_triadic = signed_triadic_growth_graph(triadic)
        return PrimeGraph(
            primes=primes,
            ...
            signed_triadic_growth=signed_triadic,
        )
