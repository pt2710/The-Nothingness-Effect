"""Generate SOInet base/source residual artifacts and provenance manifests."""
...
def fixture(seed: int = 0) -> SOInetContractInput:
    ...
    modalities = torch.stack(
        [features * (1.0 + 0.1 * index) for index in range(3)]
    )
    modalities = modalities + 0.01 * torch.randn(
        modalities.shape, generator=generator
    )
...
def run_suite(output_dir: str | Path, *, seed: int = 0):
    ...
    source_ids = {str(contract.complex_id) for contract in source_contracts()}
    rows = [
        {
    ...
    for index, modality in enumerate(spatial.local_operator.mean(dim=-1)):
        axis.plot(
            ...
            modality.detach().cpu(),
            label=f"modality {index}",
        )
    axis.set(
        title="SOInet modality-spatial defect field",
    ...
    figure = save_figure(
        figure_handle,
        output / "soinets_spatial_closure.png",
        dpi=160,
    )
    ...
    for contract, evaluation in evaluations:
        ...
        simulation = SimulationResult(
            ...
            {
                "module": "soinets",
                "fixture": "modality-field-v2",
                "modality_count": value.modalities.shape[0],
                "spatial_samples": value.modalities.shape[1],
                "source_registry": (
                    "extended_A"
                    if str(contract.complex_id) in source_ids
                    else "base"
            ...
            write_artifact_manifest(
                ...
                build_manifest(
                    ...
                    regeneration_command=(
                        ...
                        "soinets.simulation.run_contract_suite "
