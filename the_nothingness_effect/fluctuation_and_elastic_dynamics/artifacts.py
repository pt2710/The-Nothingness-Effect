...
def _contracts_and_input(category: str) -> tuple[tuple[Any, ...], Any]:
    if category == "dfi":
        ...
        expected = np.asarray(
            require_finite_dfi(
                normalized_dfi(data, spectrum_scale=7.0)
            ).normalized_entropy,
            dtype=float,
        )
        swap = np.array(
        ...
        def input_for(identifier: str):
            ...
            if identifier == "dfi_uniqueness_of_decomposition_and_mapping_ambiguity":
                return DFIDecompositionInput(data, 7.0, (2, 0, 1))
            if identifier == "dfi_flowpoint_consistency_and_interface_inconsistency":
                return DFIFlowpointInterfaceInput(data, 7.0, swap)
            if identifier == "dfi_simulation_consistency_and_simulation_breakdown":
                return DFISimulationInput(data, 7.0, expected)
            if identifier == "flowpoint_certified_dfi_validation_functional":
                return AdditiveDerivationInput(
                    {
        ...
        return base_contracts() + derived_contracts(), input_for
    if category == "elastic_pi":
        ...
        value = ElasticPiInput(
            np.array([0.2, 0.7, 1.8, 3.4, 5.5]),
            2.5,
    ...
    if category == "elastic_pi_norm":
        ...
        value = ElasticPiNormInput(
            np.array([1, 3, 8, 5, 12]),
            np.array([0.2, 0.7, 1.6, 2.1, 3.4]),
            4.0,
            anchored=False,
        )
        return contracts(), lambda _identifier: value
    ...
    *,
    ...
    for contract in contracts:
        evaluation = evaluate_contract(
            contract,
            input_for(str(contract.complex_id)),
        )
    ...
    metrics_path = save_csv(
        output / f"{category}_contract_metrics.csv",
        records,
    )
    ...
    axis.bar(
        ...
        [item["residual_norm"] for item in records],
    )
    axis.set(
        title=f"{category} theorem residuals",
        xlabel="contract index",
        ylabel="residual norm",
    )
    figure_path = save_figure(
        figure,
        output / f"{category}_contract_residuals.png",
        dpi=160,
    )
    plt.close(figure)
    ...
    for contract, evaluation in evaluations:
        ...
        simulation = SimulationResult(
            ...
            evaluation.status,
            {"category": category, "fixture": "deterministic-contract-v2"},
            seed,
            {"absolute": 1e-10},
            ...
            write_artifact_manifest(
                ...
                build_manifest(
                    ...
                    regeneration_command=(
                        f"python -m {module_name}.simulation.run_contract_suite "
