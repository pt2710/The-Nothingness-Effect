"""Source-faithful DFI C01 closure with normalized spatial defect potential."""
...
ExactSpatialDFIInput = _previous.ExactSpatialDFIInput
ExactSpatialDFIClosure = _previous.ExactSpatialDFIClosure
SOURCE_IDS = _previous.SOURCE_IDS
IMPLEMENTATION_PATH = (
    "the_nothingness_effect/fluctuation_and_elastic_dynamics/"
    "dynamic_fluctuation_index/normalized_closure_contracts.py"
)
...
def spatial_closure_operator(
    ...
    fields_1b = _previous._field_matrix(source.source_energy_fields_1b, label="DFI 1B")
    fields_2b = _previous._field_matrix(source.source_energy_fields_2b, label="DFI 2B")
    if fields_1b.shape != fields_2b.shape:
        raise _previous.DomainViolationError("DFI 1B and 2B fields must share one domain")
    weights_1b = _previous._weights(source.source_weights_1b, label="DFI 1B")
    weights_2b = _previous._weights(source.source_weights_2b, label="DFI 2B")
...
def _residual(
    ...
    return ResidualResult(
        "DFI C01 normalized-potential formula and coercivity residual",
        vector,
        source.tolerance,
        passed,
...
def _source_removal(source_id: str, source_index: int):
    def check(value: ExactSpatialDFIInput | _legacy.SpatialDFIInput) -> SourceRemovalResult:
...
def _replacement_contract() -> ComplexContract:
    return ComplexContract(
        ...
        appendix=_previous._base.APPENDIX,
        appendix_source_sha256=_previous._base.APPENDIX_SHA256,
        ...
        codomain=CodomainSpec(
            ...
            "raw-energy volume, normalized-defect gradient, boundary, coercivity, exchange, and reference certificate",
            (ExactSpatialDFIClosure,),
        ),
