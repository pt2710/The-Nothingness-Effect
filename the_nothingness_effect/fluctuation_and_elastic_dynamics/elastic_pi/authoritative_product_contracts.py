"""Byte-faithful Elastic-pi C01 product-field contract."""
...
SOURCE_IDS = (str(_base.B1), str(_base.B2))
IMPLEMENTATION_PATH = (
    "the_nothingness_effect/fluctuation_and_elastic_dynamics/"
    "elastic_pi/authoritative_product_contracts.py"
)
...
def _product_input(value: ExactProductInput | _base.ElasticPiInput) -> ExactProductInput:
    if isinstance(value, ExactProductInput):
        return value

    reciprocity = _base.dfi_curvature_operator(value)
    certification = _base.certified_simulation_operator(value)
    ...
    first_1, second_1 = _pair(
        reciprocity_residual <= value.tolerance,
        reciprocity.coupled_operator,
        np.array([reciprocity_residual, reciprocity.interaction_energy]),
    )
    first_2, second_2 = _pair(
        certification_residual <= value.tolerance,
        certification.certification_operator,
        np.array([certification_residual]),
    )
...
def product_operator(value: ExactProductInput | _base.ElasticPiInput) -> ExactProductResult:
...
def _residual(value, output):
    return exact_product_residual(
        ...
        output,
        name="Elastic-pi C01 product projections and pointwise involution",
    )
...
def _replacement() -> ComplexContract:
    return ComplexContract(
        ...
        source_ids=(_base.B1, _base.B2),
        domain=DomainSpec(
            "Elastic-pi C01 authoritative product carrier",
            "paired reciprocity and simulation-certification B states with exact projections",
            (ExactProductInput, _base.ElasticPiInput),
        ),
        codomain=CodomainSpec(
            "Elastic-pi C01 spatial product realization",
        ...
        artifact_spec=ArtifactSpec(
            ("projection_table", "curvature_source_table", "exchange_square_record"),
