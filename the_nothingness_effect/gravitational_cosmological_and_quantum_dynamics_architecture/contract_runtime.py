...
def contracts_for(spec: PhysicalModuleSpec) -> tuple[ComplexContract, ...]:
    ...
    canonical_module = CANONICAL_MODULE_IMPORTS[spec.module]
    implementation = f"{canonical_module.replace('.', '/')}/contracts.py"
    artifact = ArtifactSpec(
        ("field_csv", "residual_plot", "source_removal_table"),
        f"python -m {canonical_module}.simulation.run_contract_suite",
    )
