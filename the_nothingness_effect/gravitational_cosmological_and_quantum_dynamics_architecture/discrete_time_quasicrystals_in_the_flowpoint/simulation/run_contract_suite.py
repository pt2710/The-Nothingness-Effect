...
def _promote_source_faithful_artifacts(output: Path, legacy_output: Path) -> dict[str, object]:
    ...
    # The theorem manifests already carry their own provenance checks. They contain
    # the producing Git commit and are intentionally excluded here to avoid a
    # self-invalidating checksum cycle after committing freshly regenerated bytes.
    stable_payload = sorted(
        {
            "dtqc_contract_metrics.csv",
            ROOT_COMPATIBILITY_FIGURE,
            ROOT_METADATA_FILE,
            *PROMOTED_ARTIFACTS.values(),
        }
    )
    checksums = {
        "schema_version": "3.0",
        "algorithm": "sha256",
        "scope": "stable source-faithful root payload; theorem manifests are verified separately",
        "files": {name: _sha256(output / name) for name in stable_payload},
    }
    ...
    if final_inventory != root_inventory:
        raise RuntimeError(
            f"unexpected root DTQC artifact inventory: actual={final_inventory}, expected={root_inventory}"
        )
