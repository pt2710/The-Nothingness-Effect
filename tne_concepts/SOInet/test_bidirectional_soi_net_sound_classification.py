# test_bidirectional_soi_net_sound_classification.py
...
FIG_DIR = 'test_bidirectional_sound_classification_data/soi_net_test_figures'
DATA_DIR = 'test_bidirectional_sound_classification_data/soi_net_test_data'
AUDIO_DIR = 'test_bidirectional_sound_classification_data/audio_comparisons'
WAVEFORM_DIR = 'test_bidirectional_sound_classification_data/sound_wave_comparisons'
...
def main():
    ...
    metrics_df.to_csv(f'{DATA_DIR}/class_metrics.csv', index=False)
    macro_df.to_csv(f'{DATA_DIR}/macro_metrics.csv', index=False)
    pd.DataFrame({'accuracy':[acc]}).to_csv(f'{DATA_DIR}/accuracy.csv', index=False)
    ...
    for i, (input_freq, predicted_class) in enumerate(zip(freqs, pred)):
        out_path = f"{AUDIO_DIR}/test_{i:03d}_input_{int(input_freq)}Hz_class_{predicted_class}_{prototypes[predicted_class]}Hz.wav"
    ...
    for i, (
        ...
        spa.plot_spectrum_classification(freqs, prototypes, i, fig_dir=FIG_DIR)
