...
def build(status_path: Path, provenance_path: Path, csv_output: Path, json_output: Path) -> dict[str, object]:
    ...
    for identifier, status_record in sorted(status_by_id.items()):
        ...
        if closure not in OPEN_STATUSES:
            continue
        manifest = manifest_by_id.get(identifier)
        if manifest is None:
            raise RuntimeError(f"missing provenance for open contract: {identifier}")
    ...
    report = {
        ...
        "counts": counts,
        "all_open_states_represented": len(rows)
        == int(status_payload.get("open_and_numerical_candidate_preserved", -1)),
...
def main() -> int:
    ...
    parser.add_argument(
        "--require-zero",
        action="store_true",
        help="fail unless no OPEN or NUMERICAL_CANDIDATE records remain",
    )
    ...
    if args.require_zero and report["open_or_numerical_candidate_count"] != 0:
        raise RuntimeError(
            "mathematical closure gate failed: "
            f"{report['open_or_numerical_candidate_count']} OPEN/NUMERICAL_CANDIDATE records remain"
        )
    return 0
