...
def build(arguments: argparse.Namespace) -> dict[str, Any]:
    ...
    if not bool(equation_alignment.get("passed")):
        blockers.append("equation_implementation_alignment_failed")
    if int(equation_alignment.get("rows", 0)) != total:
        blockers.append("equation_implementation_row_mismatch")
    if int(equation_alignment.get("complete_mappings", 0)) != total:
        blockers.append("equation_implementation_mapping_incomplete")
    if int(equation_alignment.get("source_faithful_corrections", 0)) != 23:
        blockers.append("source_faithful_correction_mapping_mismatch")
    if equation_alignment.get("errors"):
        blockers.append("equation_implementation_errors_present")
    ...
    payload = {
        ...
        "authority_archive_expected_sha256": archive_qa.get("expected_archive_sha256"),
        ...
        "theorem_artifact_coverage": {
            ...
            "diagnostic_bundles_enriched": artifact_coverage.get("diagnostic_bundles_enriched"),
        ...
        "equation_implementation_alignment": {
            "rows": equation_alignment.get("rows"),
            "complete_mappings": equation_alignment.get("complete_mappings"),
            "source_faithful_corrections": equation_alignment.get("source_faithful_corrections"),
            "first_labels": equation_alignment.get("first_labels"),
            "equation_labels": equation_alignment.get("equation_labels"),
            "passed": equation_alignment.get("passed"),
        },
