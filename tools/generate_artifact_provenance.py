...
def _implemented_ids(matrix: Path) -> set[str]:
    with matrix.open(newline="", encoding="utf-8-sig") as handle:
        rows = bind_inventory_rows(list(csv.DictReader(handle)))
    return {
        row["complex_id"]
        for row in rows
...
def _animation_generators() -> list[Path]:
    ...
    candidates = {
        ...
        *Path("the_nothingness_effect/artificial_intelligence").glob(
            "**/simulation/run_all_capabilities.py"
        ),
        *Path("the_nothingness_effect/artificial_intelligence").glob(
...
def generate(
    ...
    for manifest in manifests:
        manifest["regeneration_command"] = manifest[
            "regeneration_command"
        ].replace(output_token, "<output-root>")
    ...
    payload = {
        ...
        "local_artifact_policy": (
            ...
            "producer-local metrics, manifests, representative figures, "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--aggregate",
        type=Path,
        default=Path("docs/data/artifact_provenance_manifest.json"),
    )
