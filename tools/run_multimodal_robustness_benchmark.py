...
CLAIM_BOUNDARY = (
    "synthetic multi-seed robustness evidence; "
    "not empirical validation or theorem proof"
)
...
def run(
            ...
            rows.append(
                {
                    ...
                    **evaluation.metrics,
                    "closure_status": (
                        evaluation.output.closure_status.value
                    ),
                    "residual_l2": sum(
                        value * value
                        for value in evaluation.residuals.values()
                    )
                    ** 0.5,
                    ...
                    "final_validation_loss": (
                        training.history[-1].validation_loss
                    ),
                    "final_validation_accuracy": (
                        training.history[-1].validation_accuracy
                    ),
                }
            )
        for record in evaluate_source_removals(
            model,
            dataset.test,
        ):
    ...
    _write_csv(
        output / "multiseed_scenario_metrics.csv",
        rows,
    )
    _write_csv(
        output / "multiseed_metric_summary.csv",
        aggregate,
    )
    _write_csv(
        output / "multiseed_source_removal.csv",
        ablation_rows,
    )
    ...
    axis.errorbar(
        range(len(accuracy)),
        [float(item["mean"]) for item in accuracy],
        yerr=[
            float(item["population_std"])
    ...
    axis.set_xticks(
        range(len(accuracy)),
        [str(item["scenario"]) for item in accuracy],
        rotation=35,
        ha="right",
    )
    ...
    figure.savefig(
        output / "multiseed_accuracy_robustness.png",
        dpi=160,
    )
    plt.close(figure)
    ...
    axis.scatter(
        ...
        [
            float(row["mean_reconstruction_rmse"])
            for row in clean
        ],
    )
    for row in clean:
    ...
    figure.savefig(
        output / "seed_calibration_reconstruction.png",
        dpi=160,
    )
    plt.close(figure)

    module_output = output / "module_training_evaluation"
    module_report = run_ai_module_training_evaluation(
        module_output,
        seeds=seeds,
        epochs=epochs,
        samples_per_class=10,
    ...
    report = {
        ...
        "all_metrics_finite": all(
            torch.isfinite(
                torch.tensor(float(row[name]))
            )
            for row in rows
            for name in metric_names
        ),
        "module_training_evaluation": {
            "path": "module_training_evaluation",
            "training_status": module_report[
                "training_status"
            ],
            "validation_status": module_report[
                "validation_status"
            ],
            "closure_status": module_report[
                "closure_status"
            ],
            "all_metrics_finite": module_report[
                "all_metrics_finite"
            ],
        },
        "outputs": [
            "multiseed_scenario_metrics.csv",
            "multiseed_metric_summary.csv",
            "multiseed_source_removal.csv",
            "multiseed_accuracy_robustness.png",
            "seed_calibration_reconstruction.png",
            "module_training_evaluation/",
        ],
    }
    (
        output / "multiseed_robustness_manifest.json"
    ).write_text(
        json.dumps(
            report,
            indent=2,
    ...
    if not report["all_metrics_finite"]:
        raise RuntimeError(
            "non-finite robustness metric"
        )
    if not module_report["all_metrics_finite"]:
        raise RuntimeError(
            "non-finite module evaluation metric"
        )
    return report
...
def main() -> int:
    ...
    print(
        "multimodal_robustness=passed "
        f"seeds={report['seeds']} "
        f"records={report['records']} "
        "module_training_evaluation=passed"
    )
    return 0
